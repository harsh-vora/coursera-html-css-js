...
if (process.env.__NEXT_I18N_SUPPORT) {
    ...
    if (locales) {
        ...
        } else { // derive the default locale if it wasn't detected in the asPath
...
if (window.__NEXT_P) { // Defer page registration for another tick. This will increase the overall
...
var _default = async (opts = {}) => { // This makes sure this specific lines are removed in production
    ...
    } catch (error) { // This catches errors like throwing in the top level of a module
        initialErr = error;
    }
    if (process.env.NODE_ENV === 'development') {
        const {
        ...
        if (initialErr) {
            if (initialErr === hydrateErr) {
                setTimeout(() => {
                    let error;
                    try { // Generate a new error object. We `throw` it because some browsers
...
async function render(renderingProps) {
    ...
    } catch (renderErr) { // bubble up cancelation errors
        if (renderErr.cancelled) {
            throw renderErr;
        }
        if (process.env.NODE_ENV === 'development') { // Ensure this error is displayed in the overlay in development
            setTimeout(() => {
                throw renderErr;
            });
        }
...
let reactRoot = null;
let shouldHydrate = typeof _reactDom.default.hydrate === 'function';

function renderReactElement(domEl, fn) { // mark start of hydrate/render
    if (_utils.ST) {
    ...
    if (process.env.__NEXT_REACT_ROOT) {
        ...
        shouldHydrate = false;
    } else { // The check for `.hydrate` is there to support React alternatives like preact
        if (shouldHydrate) {
...
function Root({
    ...
    if (process.env.__NEXT_TEST_MODE) { // eslint-disable-next-line react-hooks/rules-of-hooks
