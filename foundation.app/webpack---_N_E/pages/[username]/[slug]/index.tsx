...
import { getArtworkPageProps } from 'queries/server/artwork-page';
...
export default function ArtworkIndexPage(
  ...
  const { artwork, percentSplits } = props;
  ...
  const { name, description, tokenId } = artwork;

  const openGraphAsset: string = buildPageShareUrl(artwork);
  const posterUrl: string = buildPosterUrl(artwork);
