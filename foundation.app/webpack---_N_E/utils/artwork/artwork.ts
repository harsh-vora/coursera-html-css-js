...
import Artwork, { ArtworkV2, PolymorphicArtwork } from 'types/Artwork';
...
export const getArtworkTokenId = (artwork: PolymorphicArtwork): string =>
...
export const getSlug = (product: PolymorphicArtwork): string => {
...
export const getTokenSymbolFromProduct = (
  product: PolymorphicArtwork
): string => {
...
export function buildCreatorArtworkPath(artwork: PolymorphicArtwork): string {
  const artworkSymbol = getTokenSymbolFromProduct(artwork);
...
const sortByHasDateMinted = comparator((a: Artwork) => Boolean(a.dateMinted));
export const getDateMinted = compose(Number, prop('dateMinted'));

export const sortByDateMinted = sortWith<Artwork>([
  // ascend artworks by the dateMinted prop
  descend(getDateMinted),
  // then sort artworks with dateMinted prop to be first
  sortByHasDateMinted,
]);
...
export const isArtworkListed = allPass([
  (artwork: Artwork) => Boolean(artwork?.mostRecentActiveAuction),
  (artwork) => isAuctionStatusOpenOrFinalized(artwork?.mostRecentActiveAuction),
]);
...
export const getComputedArtworkStatusV2 = ({
  ...
  return cond<EventType, ComputedArtworkStatus>([
    ...
    [
      (status) => includes(status, [EventType.Unlisted]),
      always(ComputedArtworkStatus.Minted),
    ],
    [
