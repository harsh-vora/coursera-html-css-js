...
export default function TransactionNavigation(
  ...
  const stepCountForNavigation = navigationSteps.length - 1;

  const currentStep = useNavigationFlow((state) => state.currentStep);

  const indexedStep = currentStep - 1;
  const roundedIndexedStep = Math.round(indexedStep);

  const progressPercentage = (indexedStep / stepCountForNavigation) * 100;
    ...
    >
      ...
      <Grid sx={sx.steps}>
        {navigationSteps.map((navigationStep, key) => {
          const isCurrentStep = roundedIndexedStep === key;
          return (
