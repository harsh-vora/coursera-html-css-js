...
import { getUserArtworksCreated } from 'hooks/queries/hasura/use-user-artworks-created';
import { getUserArtworksCollected } from 'hooks/queries/hasura/use-user-artworks-collected';
...
export async function getUserForProfile(
  ...
  if (isAddress) {
    const [user, collectors, createdCount] = await Promise.all([
